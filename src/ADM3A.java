...
public class ADM3A extends JFrame implements Typer, KeyListener, MouseListener,
	...
	static final int M_ESC = 1;
	static final int M_SETCURS1 = 2;
	static final int M_SETCURS2 = 3;
	...
	public ADM3A(Properties props, TermContainer fe) {
		...
		paster = new Paster(props, "adm3a", this);
		bell = new Beep(props, "adm3a");
	...
	public void process_keychar(int c) {
		...
		if (mode == M_ESC) {
			...
			mode = M_NORMAL;
			return;
		} else if (mode == M_SETCURS1) {
			set_curs_y((c - ' ') & 0xff);
			mode = M_SETCURS2;
			return;
		} else if (mode == M_SETCURS2) {
			set_curs_x((c - ' ') & 0xff);
			crt.setCursor(curs_x, curs_y);
			mode = M_NORMAL;
			return;
